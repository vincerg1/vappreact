...
const Ticket = ({ order }) => {
  ...
  useEffect(() => {
    const generateQR = async () => {
      try {
        const data = getQRCodeData(); // tu función ya existente
        const qrImage = await QRCode.toDataURL(data); // genera base64
        setQrData(qrImage); // guarda en estado
      } catch (error) {
    ...
    return (
      ...
      <div className="ticket">
        ...
        <div className="section details">
          ...
          {order.partners && order.partners.length > 0 && (
            <div className="producto-item">
              <div style={{ fontWeight: 'bold', marginTop: '0.5rem' }}>Complementos:</div>
              <ul>
                {order.partners.map((item, idx) => (
                  <li key={idx}>Cant: {item.cantidad} / {item.producto}</li>
                ))}
              </ul>
            </div>
          )}
        ...
        <div className="section qr-code">
          ...
          {qrData && <img src={qrData} alt="QR" style={{ width: '100px', height: '100px' }} />}
        </div>
    
        <div className="section express-info">
