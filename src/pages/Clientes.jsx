...
export default function Clientes() {
  ...
  return (
    <li className="cliente-item">
      ...
      <span>{cliente.email} - {cliente.bday}</span>
  ...
  function FormularioCliente({ onClienteAgregado }) {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        const response = await axios.post('http://localhost:3001/clientes', { email, bday, password });
        console.log('Cliente agregado con éxito:', response.data);
  
        // Aquí está el manejo del id_cliente recibido
        const { id_cliente } = response.data;
        console.log('Nuevo ID del cliente:', id_cliente);
